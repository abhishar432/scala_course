...
object TweetSet {
  val TweetNil: Tweet = new Tweet("", "", -1)
}
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def descendingByRetweet: TweetList = {
    try {
      val r = mostRetweeted
      new Cons(r, remove(r).descendingByRetweet)
    } catch {
  ...
  def mostRetweeted: Tweet = {
    def getMostTweeted(s: TweetSet): Tweet = {
      try {
        s.mostRetweeted
      } catch {
        case ex: NoSuchElementException =>
    ...
    val lmr = getMostTweeted(left)
    val rmr = getMostTweeted(right)

    if (lmr.retweets > rmr.retweets) {
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = {
    tweetsMentioning(google)
  }
  lazy val appleTweets: TweetSet = {
    tweetsMentioning(apple)
  }
